...
class FoodSpider(scrapy.Spider):
    name = "food"
    allowed_domains = ["mapi.dianping.com", "m.dianping.com", "restapi.amap.com"]
    ...
    # start_urls = ['http://mapi.dianping.com/searchshop.json?start=0&regionid=0&categoryid=0&sortid=0&cityid=%s' % 7]

    def __init__(self, city=None, province=None, city_id=None, cat_id=None, table_name=None, *args, **kwargs):
        super(FoodSpider, self).__init__(*args, **kwargs)
        self.city_id = int(city_id)
        self.pro = str(province).encode('utf-8').decode('unicode_escape')
        self.city = str(city).encode('utf-8').decode('unicode_escape')
        self.food_id = int(cat_id)
        self.table_name = table_name
        print("ss")

        # self.start_urls = ['http://mapi.dianping.com/searchshop.json?start=0&regionid=0&categoryid=0&sortid=0&cityid=%s' % city_id]
        # print(self.start_urls)

    def start_requests(self):
        url = 'http://mapi.dianping.com/searchshop.json?start=0&regionid=0&categoryid=0&sortid=0&cityid=%s' % self.city_id
        print(url)
    ...
    def parse_map(self, response):
        ...
        if item['center'] != "":
            url = "http://restapi.amap.com/v3/geocode/regeo?key=%s&location=%s&radius=100&extensions=base&batch=false&roadlevel=1" % (self.key, item['center'])
            response.meta['item'] = item
            yield scrapy.Request(url=url, callback=self.parse_district, meta=response.meta)
        else:
            yield item

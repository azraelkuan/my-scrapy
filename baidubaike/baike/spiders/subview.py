...
import json
import logging
from baike.items import BaiduBaikeItem
from scrapy.selector import Selector


class SubviewSpider(scrapy.Spider):
    name = "subview"
    ...
    def parse_item(self, response):
        ...
        try:
            ...
            for sub_url in url_list:
                ...
                SubviewSpider.log(self, "need to get sub baidu item, url is %s" % full_url, level=logging.WARNING)
                self.sub_num += 1
                yield scrapy.Request(url=full_url, callback=self.parse_item, meta={'i': response.meta['i'], 'sub': True})
    # 初始设置pv为0
        item['pv'] = 0

        if response.meta['i'] % 1000 == 0:
            print(response.meta['i'])
            SubviewSpider.log(self, "sub num is %s, error num is %s " % (self.sub_num, self.error_num),
                              level=logging.INFO)

        if "sub" in response.meta:
            if item['item_id'] != 0:
                ...
                try:
                    ...
                    SubviewSpider.log(self, "basic info error, url is %s" % response.url, level=logging.WARNING)
                    item['basic_info'] = {}
                # 所有信息
                try:
                    ...
                    SubviewSpider.log(self, "all info error, url is %s" % response.url, level=logging.WARNING)
                    item['level2'] = {}
